...
**Never commit `.credentials.json` or tokens to the repo.** The `.env` file is gitignored for local overrides.

### Environment

Project-level env vars are set in `.claude/settings.local.json` under the `env` key (not committed). This avoids needing `export PATH=...` in every Bash tool invocation. The `settings.local.json` file is gitignored via `.claude/` — use it for machine-specific config like PATH.
